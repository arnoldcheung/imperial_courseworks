...
class PricingModel():
    ...
    def _preprocessor(self, X_raw, mode='train', pca=False):
       ...
       'town_mean_altitude', 'town_surface_area', 'population', 'commune_code', 'canton_code', 'city_district_code', 'regional_department_code']
        ...
        data_cat = data.loc[:, ['pol_coverage', 'pol_payd', 'pol_usage', 'drv_sex1', 'vh_fuel', 'vh_type', 'drv_sex2', 'pol_pay_freq']]
        data_cont = data.drop(columns=['pol_coverage', 'pol_payd', 'pol_usage', 'drv_sex1', 'vh_fuel', 'vh_type', 'drv_sex2', 'pol_pay_freq'])
         
        # replace NA by the median of the column
        data_cont.fillna(data.median(), inplace=True)
        
        if pca:
            if mode=='train':
                X_cont=self.pca.fit_transform(self.ss.fit_transform(data_cont.values))
            else:
                X_cont=self.pca.transform(self.ss.transform(data_cont.values))
        else:
            if mode=='train':
                X_cont=self.ss.fit_transform(data_cont.values)
            else:
                X_cont=self.ss.transform(data_cont.values)
        ...
        data_cat.pol_pay_freq = data_cat.pol_pay_freq.replace(values_pol_pay_freq)
        # One Hot Encoding
#        data_cat=data_cat.drop(columns=['pol_coverage', 'vh_fuel', 'pol_payd', 'vh_type'])
#        cat_cols = ['pol_usage', 'drv_sex1']
        cat_cols = ['pol_coverage', 'pol_payd', 'pol_usage', 'drv_sex1', 'vh_fuel', 'vh_type']
        
        
        data_cat = pd.get_dummies(data_cat, prefix=cat_cols, columns=cat_cols, dummy_na=False)
        data_cat = pd.get_dummies(data_cat, prefix=['drv_sex2'], columns=['drv_sex2'], dummy_na=True)
        ...
        X_clean=np.concatenate([X_cont, X_cat], axis=1)
    ...
    def predict_claim_probability(self, X_raw):
        ...
        X_clean = self._preprocessor(X_raw, mode='test')
